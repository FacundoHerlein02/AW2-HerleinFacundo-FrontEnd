...
export const filtro= marcas.map(([marca,ruta])=>`
            <div class= "active:scale-95 bg-slate-100 w-20 h-20 rounded-md aspect-video cursor-pointer" data-marca="${marca}">
                <img title="${marca}" class="w-full h-full object-contain"src="${ruta}" alt="${marca}">
            </div>
`).join('');